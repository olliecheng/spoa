...
namespace spoa {
  ...
  Graph::AlignmentResult Graph::AddAlignment(
    ...
    std::uint32_t weight) {
    return AddAlignment(alignment, sequence.c_str(), sequence.size(), weight);
  }
  ...
  Graph::AlignmentResult Graph::AddAlignment(
    ...
    return AddAlignment(alignment, sequence, sequence_len, weights);
  }
  ...
  Graph::AlignmentResult Graph::AddAlignment(
    ...
    if (alignment.empty()) {
      ...
      // in this case, valid_nodes should be equal to the sequence length:
      // all the nodes are valid, but not aligned!
      return Graph::AlignmentResult{ sequence_len, sequence_len, sequence_len };
    }

    // store all the bases in the new sequence that are aligned to 
    ...
    std::uint32_t valid_nodes = valid.size();
    ...
    // add aligned bases
    for (const auto& it : alignment) {
      // if there is no node in the new sequence, no modifications are needed
      if (it.second == -1) {
        continue;
      }
      ...
      if (it.first == -1) {
        ...
        auto jt = nodes_[it.first].get();
        if (jt->code == code) {
          // if the graph and the sequence are the same, we can merge these nodes
          ...
          // we have to create a new node, as this is aligned but not identical
          if (!curr) {
            new_nodes++;
