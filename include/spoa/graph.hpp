...
namespace spoa {
  ...
  // Alignment is a vector of pairs. The first element is the index of the GRAPH node position,
  // the second is the index of the SEQUENCE node position.
  ...
  public:
    ...
    struct AlignmentResult {
      std::uint32_t new_nodes;
      std::uint32_t sequence_len;
      std::uint32_t valid_nodes;
    };

    AlignmentResult AddAlignment(
